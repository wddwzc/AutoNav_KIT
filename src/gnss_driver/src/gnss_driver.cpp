	...
	while(ros::ok())
	{
		...
		if(n>0)
		{
			...
			else if(n > 0)
			{
				...
				int retHeading = gnss_headingData.CHeadingData((char*)buffer); 
				//std::cout<<retHeading<<" =="<<std::endl;					
				//这步是解析heading语句！！！！！！！！！！！
				//==刚开始的几个，容易有问题，去掉==
				if(!sysInit)
				{
				...
				if(retHeading> 0)//这里是额外加的，用于接收显示的
				{
					...
					myout<<std::fixed<<"base line "<<showBaseLine<<" Yaw "<<showYawAngle<<" Pitch "<<showPitchAngle<<std::endll;
					myout.close();	
#endif
