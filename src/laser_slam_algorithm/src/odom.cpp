	...
	while (status)
	{
			...
			fabs(timeImuTrans - timeSurfPointsLessFlat) < 0.005)
		{
			...
			if (!systemInited)
			{
				...
				//点云的初始位姿，就是这里决定的，使用最开始的IMU来给姿态，yaw不要了
			...
			if (laserCloudCornerLastNum > 10 && laserCloudSurfLastNum > 100)
			{
				...
				for (int iterCount = 0; iterCount < 25; iterCount++)
				{
				   ...
				   for (int i = 0; i < cornerPointsSharpNum; i++)
				   {
						...
						if(!pcl_isfinite(pointSel.x) || !pcl_isfinite(pointSel.y) || !pcl_isfinite(pointSel.z))   //有可能转换后为无效点，程序会崩
						{
							pointSearchCornerInd1[i] = -1;
							pointSearchCornerInd2[i] = -1;
							continue;
						}
					...
					for (int i = 0; i < surfPointsFlatNum; i++)
					{
						...
						if(!pcl_isfinite(pointSel.x) || !pcl_isfinite(pointSel.y) || !pcl_isfinite(pointSel.z))  //有可能转换后为无效点，程序会崩
						{
							pointSearchSurfInd1[i] = -1;
							pointSearchSurfInd2[i] = -1;
							pointSearchSurfInd3[i] = -1;
							continue;
						}
					...
					matAtA = matAt * matA;
					matAtB = matAt * matB;
					cv::solve(matAtA, matAtB, matX, cv::DECOMP_QR);//求解线性方程组

					//有时求解所得的旋转平移矩阵 是无效值， 跳过继续求解即可
